...
기존의 음악 스트리밍 서비스는 단순한 재생 기록만 제공할 뿐,
개인적인 코멘트나 날짜별 기록 기능을 충분히 지원하지 않는다.
본 서비스는 음악 청취 기록, 평론·평점, 커뮤니티 기능을 결합한
취향 중심 음악 플랫폼이다.

사용자는 감상한 음악을 날짜별로 기록하고 평점과 평론을 남기는 과정을 통해,
단순 청취에 그치지 않고 자신의 음악 취향을 언어화하고 정립할 수 있다.
나아가 다른 사용자의 평론을 탐색하고 유사 음원을 추천받음으로써
취향 기반의 음악 발견 경험을 제공하는 것을 목표로 한다.
...
- 사용자가 음원에 대한 평점과 평론을 작성하고 다른 사용자와 공유할 수 있어야 한다.
- 사용자가 특정 음원의 평론 목록을 조회할 수 있어야 한다.
- 사용자가 특정 음원에 유사 음원을 추천하고 추천 목록을 조회할 수 있어야 한다.
...
- Write TrackReview 평점·평론·추천곡 작성
- Edit TrackReview 평점·평론·추천곡 수정
- Delete TrackReview 평점·평론·추천곡 삭제
- View Track Reviews 트랙 평론 목록 조회 (추천곡 포함)
- View User Profile 유저 프로필 조회
- Follow User 팔로우
- Unfollow User 언팔로우
...
| **Use case #10 : Write TrackReview** |                                                                          |
|---|--------------------------------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                                          |
| Summary | 사용자가 음원에 대한 평점, 평론, 추천곡을 하나의 리뷰로 작성하기 위한 기능 |
| Scope | MusicJournal                                                             |
| Level | User Level                                                               |
| Author | 김대현                                                                      |
...
| Primary Actor | User                                                                     |
| Preconditions | 사용자가 로그인 상태여야 한다.                                                        |
| Trigger | 사용자가 평점·평론·추천곡을 입력하고 작성 버튼을 클릭할 때                                         |
| Success Post Condition | 리뷰가 DB에 저장된다. 해당 음원에 기존 리뷰가 있을 경우 덮어쓴다.                                   |
| Failed Post Condition | 리뷰 저장에 실패하고 DB에 데이터가 저장되지 않는다.                                            |
| **MAIN SUCCESS SCENARIO** |                                                                          |
| Step | Action                                                                   |
| S | 사용자가 음원에 대한 리뷰를 작성한다.                                                    |
| 1 | 이 Use case는 사용자가 평점·평론·추천곡을 입력하고 작성 버튼을 클릭할 때 시작된다.                        |
| 2 | 사용자는 평점(필수), 평론(선택), 추천곡 리스트(선택, 최대 3개)를 입력하고 작성 버튼을 클릭한다.               |
| 3 | 시스템은 입력 데이터의 유효성을 검사한다 (평점 범위 0.1~10.0, 추천곡 최대 3개 확인).                    |
| 4 | 시스템은 해당 음원에 대한 현재 사용자의 기존 리뷰 존재 여부를 DB에서 확인한다.                             |
| 5 | 시스템은 리뷰(평점·평론)를 저장하고, 추천곡 리스트가 있을 경우 함께 저장한다.                              |
...
| 3 | 3b. 추천곡이 3개를 초과한 경우                                                       |
| | 　...3b1. 시스템은 "추천곡은 최대 3개까지 등록할 수 있습니다." 오류 메시지를 반환한다.                    |
| | 　...3b2. 사용자는 추천곡을 수정하여 다시 시도한다. (Step 2)                                |
| 4 | 4a. 기존 리뷰가 존재하는 경우                                                        |
| | 　...4a1. 시스템은 기존 리뷰를 새 데이터로 덮어쓴다. (upsert)                               |
| | 　...4a2. 기존 추천곡은 전부 삭제되고 새 추천곡 리스트로 교체된다.                                |
| **RELATED INFORMATION** |                                                                          |
| Performance | ≤ 1 Seconds                                                              |
| Frequency | 사용자당 음원별 1회                                                              |
| Concurrency | 제한 없음                                                                    |
| Due Date |                                                                          |

---

| **Use case #11 : Edit TrackReview** |                                                                        |
|---|----------------------------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                                        |
| Summary | 사용자가 작성한 리뷰의 평점·평론·추천곡을 수정하기 위한 기능 |
| Scope | MusicJournal                                                         |
| Level | User Level                                                           |
| Author | 김대현                                                                  |
| Last Update | 05/06/26                                                             |
| Status | Analysis                                                             |
| Primary Actor | User                                                                 |
| Preconditions | 사용자가 로그인 상태이고 해당 음원에 본인의 리뷰가 존재해야 한다.                                  |
| Trigger | 사용자가 수정할 내용을 입력하고 수정 버튼을 클릭할 때                                         |
| Success Post Condition | 리뷰가 수정된 내용으로 업데이트된다. 추천곡은 새 리스트로 전체 교체된다.                              |
| Failed Post Condition | 리뷰 수정에 실패하고 DB의 데이터는 변경되지 않는다.                                         |
| **MAIN SUCCESS SCENARIO** |                                                                        |
| Step | Action                                                               |
| S | 사용자가 본인의 리뷰를 수정한다.                                                  |
| 1 | 이 Use case는 사용자가 수정할 내용을 입력하고 수정 버튼을 클릭할 때 시작된다.                      |
| 2 | 사용자는 평점(필수), 평론(선택), 추천곡 리스트(선택, 최대 3개)를 입력하고 수정 버튼을 클릭한다.           |
| 3 | 시스템은 입력 데이터의 유효성을 검사한다 (평점 범위 0.1~10.0, 추천곡 최대 3개 확인).                |
| 4 | 시스템은 해당 음원에 대한 본인 리뷰가 DB에 존재하는지 확인한다.                                 |
| 5 | 시스템은 리뷰(평점·평론)를 수정하고, 기존 추천곡을 전부 삭제 후 새 추천곡 리스트로 교체한다.               |
...
| **Use case #12 : Delete TrackReview** |                                                          |
|---|------------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                          |
| Summary | 사용자가 본인의 리뷰(평점·평론·추천곡 전체)를 삭제하기 위한 기능 |
| Scope | MusicJournal                                         |
| Level | User Level                                           |
| Author | 김대현                                                  |
| Last Update | 05/06/26                                             |
...
| Success Post Condition | 리뷰와 연결된 추천곡이 함께 삭제된다.                               |
...
| 2 | 사용자는 삭제 버튼을 클릭한다.                                    |
| 3 | 시스템은 해당 음원에 대한 본인 리뷰가 DB에 존재하는지 확인한다.             |
| 4 | 시스템은 리뷰와 연결된 추천곡을 함께 삭제한다.                          |
| 5 | 시스템은 삭제 성공 응답을 반환한다.                                 |
| 6 | 이 Use case는 리뷰가 성공적으로 삭제되면 끝난다.                     |
| **EXTENSION SCENARIOS** |                                                          |
| Step | Branching Action                                     |
| 3 | 3a. 해당 음원에 본인 리뷰가 존재하지 않는 경우                        |
...
| **Use case #13 : View Track Reviews** |                                                          |
|---|------------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                          |
| Summary | 사용자가 특정 음원에 대한 전체 리뷰 목록을 조회하기 위한 기능 |
| Scope | MusicJournal                                         |
| Level | User Level                                           |
| Author | 김대현                                                  |
| Last Update | 05/06/26                                             |
...
| Primary Actor | User                                                 |
| Preconditions | 사용자가 로그인 상태여야 한다.                                    |
| Trigger | 사용자가 음원을 선택하여 리뷰 목록 페이지에 접근할 때                        |
| Success Post Condition | 해당 음원의 리뷰 목록(평점·평론·추천곡 포함)이 반환된다.                     |
| Failed Post Condition | 리뷰 조회에 실패하고 목록이 반환되지 않는다.                            |
| **MAIN SUCCESS SCENARIO** |                                                          |
| Step | Action                                               |
| S | 사용자가 특정 음원의 리뷰 목록을 조회한다.                            |
| 1 | 이 Use case는 사용자가 음원을 선택하여 리뷰 목록 페이지에 접근할 때 시작된다.    |
| 2 | 사용자는 리뷰 목록 조회를 요청한다.                                 |
| 3 | 시스템은 해당 음원의 전체 리뷰 목록을 DB에서 조회한다.                    |
| 4 | 시스템은 각 리뷰의 평점·평론·추천곡 목록을 포함하여 반환한다.                 |
| 5 | 이 Use case는 리뷰 목록이 반환되면 끝난다.                        |
| **EXTENSION SCENARIOS** |                                                          |
| Step | Branching Action                                     |
| 3 | 3a. 해당 음원에 등록된 리뷰가 존재하지 않는 경우                       |
| | 　...3a1. 시스템은 빈 목록을 반환한다.                           |
| | 　...3a2. 이 Use case는 종료된다.                           |
| **RELATED INFORMATION** |                                                          |
| Performance | ≤ 1 Seconds                                          |
...
| **Use case #14 : View User Profile** |                                                          |
|---|------------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                          |
| Summary | 사용자가 특정 유저의 프로필과 평론 목록을 조회하기 위한 기능 |
| Scope | MusicJournal                                         |
| Level | User Level                                           |
| Author | 김대현                                                  |
| Last Update | 05/06/26                                             |
...
| Primary Actor | User                                                 |
| Preconditions | 사용자가 로그인 상태여야 한다.                                    |
| Trigger | 사용자가 특정 유저의 프로필 페이지에 접근할 때                           |
| Success Post Condition | 해당 유저의 프로필 정보와 평론 목록이 반환된다.                          |
| Failed Post Condition | 프로필 조회에 실패하고 데이터가 반환되지 않는다.                          |
| **MAIN SUCCESS SCENARIO** |                                                          |
| Step | Action                                               |
| S | 사용자가 특정 유저의 프로필을 조회한다.                               |
| 1 | 이 Use case는 사용자가 특정 유저의 프로필 페이지에 접근할 때 시작된다.        |
| 2 | 사용자는 프로필 조회를 요청한다.                                   |
| 3 | 시스템은 해당 유저가 DB에 존재하는지 확인한다.                          |
| 4 | 시스템은 해당 유저의 프로필 정보와 평론 목록을 반환한다.                    |
| 5 | 이 Use case는 프로필 정보가 반환되면 끝난다.                        |
| **EXTENSION SCENARIOS** |                                                          |
| Step | Branching Action                                     |
| 3 | 3a. 해당 유저가 존재하지 않는 경우                                 |
...
| **Use case #15 : Follow / Unfollow User** |                                                      |
|---|--------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                      |
...
시스템의 사용자 정보를 관리하는 핵심 클래스다. 이메일(email), 암호화된 비밀번호(passwordHash), 사용자명(username) 속성을 보유한다. RecordService의 요청을 받아 사용자 정보를 제공하며, 모든 Record는 User에 종속된다.
...
Spotify Web API와의 통신을 담당하는 클라이언트 컴포넌트다. TrackService의 요청을 받아 키워드 기반 음원 검색을 Spotify API에 전달하고, 응답으로 받은 Track 목록을 파싱하여 TrackService에 반환한다. API 호출에 필요한 Spotify Access Token을 내부적으로 관리한다.
...
사용자가 특정 음원에 남기는 평점과 평론을 나타내는 엔티티다. 평점(rating), 평론(content), 생성 일시(createdAt), 수정 일시(updatedAt) 속성을 보유한다. User와 Track을 참조하며, 동일한 User와 Track 조합에 대해 unique 제약을 가진다. (TrackReview N : User 1, TrackReview N : Track 1) 하나의 TrackReview는 여러 TrackRecommendation을 포함하며, 리뷰 삭제 시 연결된 추천곡도 함께 삭제된다.

8) TrackRecommendation

TrackReview에 종속된 추천곡 단위다. 추천 대상 음원(recommendedTrack)을 참조하며, TrackReview당 최대 3개까지 등록할 수 있다. TrackReview가 삭제되면 연결된 TrackRecommendation도 함께 삭제된다. (TrackRecommendation N : TrackReview 1, TrackRecommendation N : Track 1)
...
| TrackReview | 사용자가 특정 음원에 남기는 평점과 평론. 유저당 음원 하나에 하나만 존재한다 |
| TrackRecommendation | TrackReview에 종속된 추천곡 단위. 리뷰당 최대 3개까지 등록할 수 있다 |
