...
|---|------------------------------------------------------|
| **GENERAL CHARACTERISTICS** |                                                          |
| Summary | 사용자가 평론 탭 홈에서 이번 달 화제의 앨범 목록을 탐색하기 위한 기능 |
| Scope | MusicJournal                                         |
| Level | User Level                                           |
| Author | 김대현                                                  |
...
그림 4-2는 회원가입 화면이다. 이메일, username(@아이디), 비밀번호를 입력하여 계정을 생성한다. username은 가입 후 변경이 불가능하며 이를 화면에 명시한다. 이용약관 동의 후 가입하기 버튼으로 계정을 생성한다.
...
그림 4-5는 기록 수정 화면이다. 하단 시트(Bottom Sheet) 형태로 노출되며, 트랙은 변경이 불가하고 날짜와 코멘트만 수정할 수 있다.
...
그림 4-9는 평론 탭 홈 화면이다. 상단 검색창에서 앨범을 검색할 수 있으며, 이번 달 화제의 앨범이 그리드 형태로 표시된다.
