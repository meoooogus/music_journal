...
본 프로젝트는 사용자가 음악을 단순히 소비하는 것을 넘어,
청취 기록과 평론·평점을 통해 자신의 취향을 언어화하고 정립할 수 있는
음악 중심 커뮤니티 서비스를 제공한다.

사용자는 날짜별 청취 일기를 기록하는 것에 더해,
음원에 대한 평점과 평론을 남기고 이를 다른 사용자와 공유할 수 있다.
또한 같은 곡을 들은 사용자들이 유사 음원을 추천함으로써
자연스러운 취향 기반의 음악 발견 경험을 제공한다.
...
- 음원에 대한 평점과 평론을 남길 수 있어야 한다
- 공개된 평론을 다른 사용자와 공유할 수 있어야 한다
- 같은 음원을 들은 사용자들이 유사 음원을 추천할 수 있어야 한다
...
| Actor | Description                                        |
|:------|:---------------------------------------------------|
| User  | 사용자는 음원에 대한 평점·평론·추천곡을 하나의 리뷰로 작성한다 |

- 평점/평론/추천곡 수정

| Actor | Description                              |
|:------|:-----------------------------------------|
| User  | 사용자는 작성한 리뷰의 평점·평론·추천곡을 수정한다 |

- 평점/평론/추천곡 삭제

| Actor | Description                              |
|:------|:-----------------------------------------|
| User  | 사용자는 작성한 리뷰를 삭제한다 |

- 평론 목록 조회

| Actor | Description                              |
|:------|:-----------------------------------------|
| User  | 사용자는 특정 음원의 전체 평론 목록을 조회한다 |
...
13. 평점/평론/추천곡 작성
- **Purpose**: 사용자가 음원에 대한 평점·평론·추천곡을 하나의 리뷰로 남길 수 있도록 한다
- **Approach**: 사용자는 평점(필수, 0.1~10.0), 평론(선택), 추천곡 리스트(선택, 최대 3개)를 입력하여 작성 요청하고, 시스템은 해당 음원에 대한 기존 리뷰 여부를 확인한 뒤 저장한다
- **Dynamics**: 동일 음원에 대한 기존 리뷰가 있을 경우 덮어쓰기(upsert) 처리된다. 추천곡은 최대 3개까지 등록할 수 있다
- **Goals**: 사용자가 음원에 대한 주관적 평가를 언어화하고 취향을 정립할 수 있도록 한다

14. 평점/평론/추천곡 수정
- **Purpose**: 사용자가 작성한 리뷰의 평점·평론·추천곡을 수정할 수 있도록 한다
- **Approach**: 사용자는 수정할 내용을 입력하고 수정 요청한다
- **Dynamics**: 시스템은 기존 리뷰를 새 데이터로 업데이트한다. 추천곡은 기존 목록을 전부 삭제하고 새 리스트로 교체한다
- **Goals**: 사용자가 자신의 리뷰를 상황에 맞게 유연하게 관리할 수 있도록 한다

15. 평점/평론/추천곡 삭제
- **Purpose**: 사용자가 본인의 리뷰(평점·평론·추천곡 전체)를 삭제할 수 있도록 한다
- **Approach**: 사용자는 삭제 요청을 수행한다
- **Dynamics**: 시스템은 리뷰와 연결된 추천곡을 함께 삭제한다
- **Goals**: 사용자가 불필요한 리뷰를 정리할 수 있도록 한다

16. 평론 목록 조회
- **Purpose**: 사용자가 특정 음원에 대해 다른 사용자들이 남긴 평론을 확인할 수 있도록 한다
- **Approach**: 사용자는 음원을 선택하고 평론 목록 조회를 요청한다
- **Dynamics**: 시스템은 해당 음원의 전체 평론 목록(평점·평론·추천곡 포함)을 반환한다
