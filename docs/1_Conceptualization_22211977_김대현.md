...
| Actor | Description                                        |
|:------|:---------------------------------------------------|
| User  | 사용자는 특정 음원을 들은 사람들에게 유사 음원을 추천한다 |

- 추천곡 조회

| Actor | Description                              |
|:------|:-----------------------------------------|
| User  | 사용자는 특정 음원의 추천곡 목록을 조회한다 |
...
13. 평점/평론 작성
- **Purpose**: 사용자가 음원에 대한 평점과 평론을 남길 수 있도록 한다
- **Approach**: 사용자는 평점(0.1~10.0)과 평론(선택)을 입력하여 작성 요청하고, 시스템은 해당 음원에 대한 기존 평론 여부를 확인한 뒤 저장한다
- **Dynamics**: 동일 음원에 대한 기존 평론이 있을 경우 덮어쓰기(upsert) 처리된다
...
- **Dynamics**: 시스템은 해당 음원의 전체 평론 목록을 반환한다
- **Goals**: 다양한 관점의 평론을 통해 음악적 취향을 넓힐 수 있도록 한다

15. 추천곡 등록
- **Purpose**: 사용자가 특정 음원을 들은 사람들에게 유사 음원을 추천할 수 있도록 한다
- **Approach**: 사용자는 기준 음원을 선택하고 Spotify 검색 또는 본인 기록에서 추천곡을 선택한다
- **Dynamics**: 시스템은 기준 음원과 추천곡의 관계를 저장한다. 동일 조합의 중복 등록은 허용되지 않는다
- **Goals**: 사용자 간 취향 기반의 음악 발견 경험을 제공한다

16. 추천곡 조회
- **Purpose**: 사용자가 특정 음원에 연결된 추천곡 목록을 확인할 수 있도록 한다
- **Approach**: 사용자는 음원을 선택하고 추천곡 목록 조회를 요청한다
- **Dynamics**: 시스템은 해당 음원에 등록된 추천곡 목록을 반환한다
- **Goals**: 청취 중인 곡과 연결된 새로운 음악을 발견할 수 있도록 한다

17. 프로필 조회
- **Purpose**: 사용자가 다른 사용자의 취향과 활동을 확인할 수 있도록 한다
- **Approach**: 사용자는 특정 유저의 프로필 페이지에 접근한다
- **Dynamics**: 시스템은 해당 유저의 프로필 정보, 평론 목록, 통계를 반환한다
...
| TrackRecommendation | 특정 음원을 들은 사용자가 유사 음원으로 추천하는 관계 단위 |
